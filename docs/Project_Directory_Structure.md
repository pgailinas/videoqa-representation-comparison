...
## Relationship Between GitHub, Hugging Face, and Google Drive

The GitHub repository, Hugging Face dataset repository, and Google Drive each serve complementary roles within the VideoQA research project.

| GitHub Repository                         | Hugging Face Dataset Repository | Google Drive Project Storage           |
| ----------------------------------------- | ------------------------------- | -------------------------------------- |
| Version-controlled notebooks              | Public tutorial ZIP archives    | Optional local archive copies          |
| Shared Python modules                     | NExT-QA benchmark video archive | Shared representation artifacts        |
| Dataset metadata and question annotations | Project artifacts archive       | Experiment-specific training artifacts |
| Project documentation                     | Public archive distribution     | Trained autoencoder models             |
| Configuration files                       | Lightweight repository support  | Autoencoder representation artifacts   |
| Pipeline diagrams                         | Large-file hosting              | VideoQA prediction artifacts           |
| Repository history                        | Anonymous public downloads      | Evaluation artifacts                   |
| Source code                               | —                               | Optional persistent project storage    |

The notebooks provide the connection between these three environments. They are maintained in the GitHub repository and may be executed in Google Colab or a local Jupyter environment. During execution, the notebooks first use locally available resources when present, then matching archives stored in a mounted Google Drive, and finally download the required public archives from the project's Hugging Face dataset repository when necessary.

Generated artifacts are always written to the local runtime environment. When Google Drive writes are enabled, selected notebooks also copy generated artifacts to the Google Drive project structure for long-term storage.

This separation keeps the GitHub repository lightweight and version-controlled, distributes the large tutorial resources through Hugging Face, and retains Google Drive as optional persistent storage for archives and generated experiment artifacts.
...
- Repository, local runtime, Hugging Face archive, and Google Drive directory paths
...
* The GitHub repository is the authoritative source for the project implementation. Large tutorial archives are distributed through the project's public Hugging Face dataset repository, while Google Drive provides optional persistent storage for local archive copies and generated experiment artifacts.

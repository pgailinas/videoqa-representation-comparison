...
- Five prediction models are supported: Cosine Similarity, Fusion MLP, Interaction Fusion, Gated Fusion, and Bilinear Fusion.
- Each prediction model produces a standardized five-score vector for every validation question. The shared prediction-generation stage selects the candidate with the highest score.
- Cosine Similarity performs direct semantic scoring without training and requires matching video and text embedding dimensions.
- The four learned fusion classifiers project video and text representations into a shared fusion space and are trained using grouped five-choice samples with cross-entropy loss.
- The learned fusion classifiers differ in how they combine the projected representations: concatenation, explicit interaction features, adaptive gating, or bilinear interactions.
